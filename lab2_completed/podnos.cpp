...
public:
	...
	for(unsigned int i = 0; i < nodesCoords.size() / 3; i++) {
            double pointX = nodesCoords[i*3]/30.0;
            double pointY = nodesCoords[i*3 + 1]/30.0;
	    meanX+=pointX;
	    meanY+=pointY;
        }
	meanX/=(nodesCoords.size() / 3.0);
	meanY/=(nodesCoords.size() / 3.0);
        for(unsigned int i = 0; i < nodesCoords.size() / 3; i++) {
            // Координаты заберём из gmsh
            double pointX = nodesCoords[i*3]/30.0 - meanX;
            double pointY = nodesCoords[i*3 + 1]/30.0 - meanY;
            double pointZ = nodesCoords[i*3 + 2]/30.0;
	    double smth = 0.0;
	    double vx = 0.0;
	    double vy = 0.0;
	    double vz = 0.0;
	    if (abs(pointZ + 0.006) < 0.001) {
		smth = sin(70*sqrt(pointX*pointX + pointY*pointY));
		vz = (1+sin(70*sqrt(pointX*pointX + pointY*pointY)))/3.0;
	    }
	...
	double tau = 1;
	    ...
	    x = nodes[i].getx();
	    y = nodes[i].gety();
            nodes[i].setvz(sin(x-tau*t)/10.0);
	    nodes[i].move(tau);
        }
    }
    ...
    void snapshot(unsigned int snap_number) {
        ...
        string fileName = "../podnos_dir/podnosik" + std::to_string(snap_number) + ".vtu";
...
int main()
{
    ...
    gmsh::initialize();
    gmsh::model::add("podnosik");

    // Считаем STL
    try {
        gmsh::merge("../поднос_ксп.stl"); 
    ...
    // ВОПРОС: неплохо бы полноценно данные сетки проверять, да?
    // ОТВЕТ: нет, это вовсе не обязательно.
